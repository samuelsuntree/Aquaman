...
public void setup(){
  ...
  upWall = new TestLine(0, 36.5f, 256, view);                 // 37 because 39 grid is the gap between the top and the begining of the tank, but it is 2 grid thick
  ...
  bottomWall = new TestLine(0, 127-38.5f, 256, view);
  wall = new BodyUnion(upWall, bottomWall);
  twin = new BodyUnion(body, wall);
  flow = new BDIM(n,m,0.69f,twin,0.00095f,true, 0);
  //
...
*/
  ...
  public void update( AbstractBody body ){
    if(body.unsteady()){get_coeffs(body);}else{ub.eq(0.f);}
    update();
  }
  public void update2( AbstractBody body ){update2();} // don't need to get coeffs again

  public void get_coeffs( AbstractBody body ){
    get_del(body);
    get_ub(body);
    get_wn(body);
  }
  
  public void get_ub( AbstractBody body ){
  ...
  public void get_wn(AbstractBody body){
  ...
  public void get_del( AbstractBody body ){
    PVector d;
    for ( int i=1 ; i<n-1 ; i++ ) {
    for ( int j=1 ; j<m-1 ; j++ ) {
      // query x-face
      d = body.del(i-0.5f,j,eps);
      del.x.a[i][j] = d.x;
      del1.x.a[i][j] = d.y;

      //query y-face
      d = body.del(i,j-0.5f,eps);
      del.y.a[i][j] = d.x;
      del1.y.a[i][j] = d.y;
    }}
    del.setBC();
...
class AbstractBody {
  public PVector del(float x, float y, float eps){return new PVector(1,0);}
  public float velocity(int d, float dt, float x, float y ){return 0;}
  public PVector WallNormal(float x, float y){return new PVector(0,0);}
  public boolean unsteady(){return false;}
}
...
class Body extends AbstractBody{
  ...
  public void end(Boolean closed, Boolean custom) {
    n = coords.size();
  ...
  public PVector del(float x, float y, float eps){
    float d = min(max(distance(x,y)/eps,-1),1);
    return new PVector(delta0(d),delta1(d)*eps);
  }
  public float delta0( float d){return 0.5f*(1.f+d+sin(PI*d)/PI);}  
  public float delta1( float d){return 0.25f*(1-sq(d))-1/TWO_PI*(d*sin(d*PI)+1/PI*(1+cos(d*PI)));}
  ...
  public void translate(PVector[] translations) {
  ...
  public PVector meanTranslation(PVector[] translations) {
    float meanX = 0;
    float meanY = 0;
  ...
  public void rotate( float dphi, PVector x1 ) {
    this.dphi = dphi;
    phi = phi+dphi;
    float sa = sin(dphi), ca = cos(dphi);
...
class CSV2DigitalTwin extends NACA {
    ...
    CSV2DigitalTwin(float x0, float y0, int m, String xFilePath, String yFilePath, String yFilteredFilePath, Window window) {
        ...
        super(x0 + 0.25f * (int) pow(2, 6) / 3, y0, (int) pow(2, 6) / 3, 0.2f, m / 2, window);
        orig = new NACA(x0 + 0.25f * (int) pow(2, 6) / 3, y0, (int) pow(2, 6) / 3, 0.2f, m / 2, window);
    ...
    public PVector WallNormal(float x, float y) { // adjust orig normal
    ...
    public int on_which_boundary_new(float x, float y) {
        if (x < (coords.get(0).x - 70) || x > coords.get(coords.size() - 1).x + 70 || y < (coords.get(0).y - 70) || y > (coords.get(0).y + 70)) {
            return PApplet.parseInt(-1); //far from edge
        } else {
            int index = 0;
            int min_index = 0;
            float dis = 1e10f, dis_temp;
    ...
    public void update(float time) { // update 'time' and coords
        ...
        int index = (int)(time / 0.686778f - 1);
        this.index = index;
        this.time = time;
    ...
    public float spaceDerivative(int a, int b, int c) {
    ...
    public float dhdx(float x, float y) {
...
class DigitalTwin extends BodyUnion{
    ...
    DigitalTwin(int chord, Window window) {
        ...
        // tail of the foil
        add(new Triangle((16-1-15/len+12.7f/len)*chord,2*chord,chord*12.71f/len,chord*62.7f/len,chord,window));
        ...
        this.len = len;
        this.chord = chord;
        this.f = 0.61f/1.375f/0.466f/chord;
    }
...
class Field{
  ...
  public float extract( float x0, float y0){
...
class NACA extends Body{
  ...
  NACA( float x, float y, float c, float t, int m, Window window) {
    this(x,y,c,t,.25f,m,window);
  }
  
  NACA( float x, float y, float c, float t, Window window ){
    this(x,y,c,t,.25f,100,window);
  }
...
class PixelBody extends AbstractBody{
  ...
  public void getArea() { // get the body area
  ...
  public PVector dpix(float i, float j){
    return new PVector(pix.interp(i+0.5f,j)-pix.interp(i-0.5f,j),pix.interp(i,j+0.5f)-pix.interp(i,j-0.5f));
  }
  public PVector del(float i, float j, float eps){return new PVector(pix.interp(i,j), dpix(i,j).mag()*eps);}
  public PVector WallNormal(float i, float j){
    PVector n = dpix(i,j);
    float m = n.mag();
    if(m>1e-8f){n.div(m);}
    return n;
...
class Rectangle extends Body{
  ...
  Rectangle(float x, float y, float l, float w, Window window){
    ...
    add(xc.x+l, xc.y+w/2);
    for ( int i=m; i>0; i-- ) {
      float theta = -TWO_PI*i/((float)m)/4.5f;
      add(xc.x+1.5f*dx*cos(theta-PI+PI/4.5f)+2*dx, xc.y+1.5f*dy*sin(theta-PI+PI/4.5f));
    }
    add(xc.x+l, xc.y-w/2);
...
class Sensor extends Body {
    ...
    Sensor(float x, float y, int chord, Window window) {
        ...
        add(xc.x+15.5f*chord/16-241*res/len,xc.y);
        ...
        //sensor 1 first side
        add(xc.x+15.5f*chord/16-242*res/len,xc.y);

        // add(xc.x+chord,xc.y);
        // add(xc.x+chord,xc.y+2);

        //sensor 1 scd side
        add(xc.x+15.5f*chord/16-242*res/len,xc.y+56*res/len);
        ...
        add(xc.x+15.5f*chord/16-241*res/len,xc.y+56*res/len);
        end();
    }
}
/**********************************
...
class Triangle extends Body {
    ...
    Triangle( float x, float y, float b, float h, float chord, Window window) {
        ...
        for ( int i=0; i<m; i++ ) {
            float theta = -TWO_PI*i/((float)m)/4.5f;
            add(xc.x+1.5f*dx*cos(theta-PI+PI/4.5f)+2*dx-7.7f*chord/74.2f, xc.y+1.5f*dy*sin(theta-PI+PI/4.5f));
        }

        // add(xc.x+h-1-7.7*chord/74.2, xc.y+0.2);
        add(xc.x+h-7.7f*chord/74.2f, xc.y);
        // add(xc.x+h-1-7.7*chord/74.2, xc.y-0.2);
